...
CREATE POLICY "Anyone can view verified help seekers" ON public.users FOR SELECT
...
CREATE POLICY "Help seekers can update their own pending profile" ON public.users FOR UPDATE
USING (
    auth.uid() = id
...
CREATE POLICY "Admins can update help seeker status" ON public.users FOR UPDATE
USING (
    (auth.jwt() ->> 'role')::text IN ('admin', 'super_admin')
);
